...
<head>
    ...
    <style>
        ...
        #category {
            ...
            padding-left: 4px;
            float: right;
            background-image: url("../Static/caret.svg");
            display: none;
        }
...
<body>
    ...
    ?>
    <div id="nav-bar">
        <img id="logo" src="../Static/logo image.png" onclick="window.open('http://localhost/Login/Nirvana/PHP/home.php', '_self')" alt="Logo">
        ...
        <div id="right-part">
            ...
            if (isset($_POST['searchButt']) && $_POST['search'] != "") {
                ...
                if (mysqli_num_rows($query) > 0) {
                    $tab = mysqli_fetch_assoc($query);
                    echo "<script style='display:none;'>window.open('http://localhost/Login/Nirvana/PHP/product.php?id_article={$tab['id_article']}', '_self');</script>";
                } else {
                    $sql = "SELECT * FROM articles WHERE name_article LIKE '%$search%' OR name_article LIKE '%$search' OR name_article LIKE '$search%'";
                    $query = mysqli_query($conn, $sql);
                    $url = "http://localhost/Login/Nirvana/PHP/searched-articles.php?";
                    $i = 1;
            ...
            ?>
            <div id="heartButt" <?php if (isset($_SESSION) && !empty($_SESSION)) echo "onclick=\"window.open('http://localhost/Login/Nirvana/PHP/wishlist.php', '_self')\"" ?>>
            ...
            <div id="cartButt" <?php if (isset($_SESSION) && !empty($_SESSION)) echo "onclick=\"window.open('http://localhost/Login/Nirvana/PHP/cart.php', '_self')\"" ?>>
    ...
    <div id="dropDown" onmouseover="dropDownMenuDisplay()" onmouseleave="dropDownMenuHide()">
        ...
        if (isset($_SESSION) && !empty($_SESSION)) {
            echo "
                    <div class='dropDownItems' id='drp1' onclick=\"window.open('http://localhost/Login/Nirvana/PHP/account.php', '_self')\"><i class='fa-solid fa-user-pen icones'></i><p class='drpText'>&ensp;Account</p></div>
                    <div class='dropDownItems' id='drp2' onclick=\"window.open('http://localhost/Login/Nirvana/PHP/shipping.php', '_self')\"><i class='fa-solid fa-truck icones'></i><p class='drpText'>&ensp;Shipping</p></div>
                    <div class='dropDownItems' id='drp3' onclick=\"window.open('http://localhost/Login/Nirvana/PHP/order%20cancellation.php', '_self')\"><i class='fa-solid fa-ban icones'></i><p class='drpText'>&ensp;Cancel</p></div>
            ...
            echo "
                <div class='dropDownItems' id='login' onclick=\"window.open('http://localhost/Login/Nirvana/PHP/login.php', '_self')\"><i class='fa-solid fa-right-to-bracket icones'></i><p class='drpText'>&ensp;Log in</p></div>
                <script>
    ...
    if (isset($_POST['logout']) && !empty($_POST['logout'])) {
        echo "<script>window.open('http://localhost/Login/Nirvana/PHP/deconnexion.php', '_self')</script>";
    }
